...
#                 right_eye_roi = face_roi[ey:ey + eh, ex:ex + ew]
#
#                 # Print shape of right eye ROI for troubleshooting
#                 print("Right Eye ROI Shape:", right_eye_roi.shape)
#
#                 # Pass right eye region through iris detection model
#                 # Perform iris detection and draw rectangles around detected iris
#
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.5:  # Confidence threshold for face detection
            ...
            for (ex, ey, ew, eh) in left_eyes:
                ...
                # Pass left eye region through iris detection model
                if not left_eye_roi.size == 0:
                    blob = cv2.dnn.blobFromImage(cv2.resize(left_eye_roi, (64, 64)), 1.0, (64, 64), (104.0, 177.0, 123.0))
                    iris_net.setInput(blob)
                    iris_landmarks = iris_net.forward()

                    if iris_landmarks.size > 0:  # Check if iris landmarks are detected
                        for landmark in iris_landmarks[0]:
                            if landmark.size == 3:  # Check if landmark contains x, y, and confidence
                                x, y = int(landmark[0] * left_eye_roi.shape[1]), int(landmark[1] * left_eye_roi.shape[0])
                                cv2.circle(left_eye_roi, (x, y), 2, (0, 0, 255), -1)
            ...
            for (ex, ey, ew, eh) in right_eyes:
                ...
                # Pass right eye region through iris detection model
                if not right_eye_roi.size == 0:
                    blob = cv2.dnn.blobFromImage(cv2.resize(right_eye_roi, (64, 64)), 1.0, (64, 64), (104.0, 177.0, 123.0))
                    iris_net.setInput(blob)
                    iris_landmarks = iris_net.forward()
                    ...
                    if iris_landmarks.size > 0:  # Check if iris landmarks are detected
                        for landmark in iris_landmarks[0]:
                            if landmark.size == 3:  # Check if landmark contains x, y, and confidence
                                x, y = int(landmark[0] * right_eye_roi.shape[1]), int(landmark[1] * right_eye_roi.shape[0])
                                cv2.circle(right_eye_roi, (x, y), 2, (0, 0, 255), -1)
